  ...
  return (
    <>
      ...
      {mobileOpen && (
        ...
        >
          <div
            className="w-80 max-w-[85vw] bg-white dark:bg-slate-900 h-full flex flex-col shadow-2xl border-r border-slate-200 dark:border-slate-800 animate-in slide-in-from-left duration-200"
      ...
      <aside className="hidden lg:flex w-72 h-full max-h-screen border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0D111A] flex-col shrink-0 font-sans transition-colors duration-200 overflow-hidden">
    ...
    <div className="flex flex-col h-full w-full justify-between font-sans overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center text-white shadow-md shadow-indigo-600/20">
        ...
        {currentTabWriteable && activeTab !== 'iam' && activeTab !== 'telegram' && (
          <button
            ...
            className="p-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-semibold flex items-center gap-1 shadow-sm transition-all hover:scale-105 active:scale-95"
            title={`Add ${activeTab}`}
          >
      ...
      <div className="p-3 space-y-2 flex-1 overflow-y-auto min-h-0 border-b border-slate-100 dark:border-slate-800/60">
        <div className="px-2 py-1 text-[10px] font-bold tracking-wider text-slate-400 dark:text-slate-400 uppercase flex justify-between">
            ...
            return (
              <button
                ...
                className={`w-full p-3 rounded-2xl transition-all duration-200 text-left flex items-center justify-between group relative ${
                  isActive
                    ? 'bg-indigo-600 text-white dark:bg-gradient-to-r dark:from-indigo-600 dark:to-purple-600 dark:text-white shadow-md shadow-indigo-600/20 font-medium'
                  ...
                  <div className="truncate">
                    ...
                    <p
                      className={`text-[10px] truncate ${
                        isActive ? 'text-indigo-100 dark:text-indigo-100' : 'text-slate-400 dark:text-slate-400'
                      }`}
                    >
                      {cat.subtitle}
                    </p>
      ...
      <div className="p-3 border-t border-slate-200 dark:border-slate-800 space-y-3 bg-slate-50/50 dark:bg-slate-900/60 shrink-0">
        {/* User Profile Card */}
        {user && (
          <button
            onClick={() => setIsProfileOpen(true)}
            className="w-full p-2.5 bg-white dark:bg-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700/60 rounded-2xl flex items-center justify-between transition-all group shadow-sm text-left"
          ...
          >
            <div className="flex items-center gap-2.5 min-w-0 pr-2">
              ...
              <div className="truncate">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[10px] text-slate-500 dark:text-slate-300 truncate font-semibold">
                    {roleName}
                  </span>
                </div>
              </div>
            ...
            <div className="p-1 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-300 transition-colors shrink-0">
        ...
        <div className="pt-1.5 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-[10px] text-slate-400 px-1">
