...
import { X, Shield, Server, Globe, Database, GitBranch, Cloud, DollarSign, Users, Send } from 'lucide-react'


interface CreateRoleModalProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (roleData: { name: string; description: string; permissions: PermissionMap }) => Promise<void>
}
...
export const CreateRoleModal: React.FC<CreateRoleModalProps> = ({ isOpen, onClose, onSubmit }) => {
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl transition-all">
        <div className="p-6 bg-gradient-to-r from-indigo-900 to-slate-900 text-white border-b border-indigo-800/40 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-extrabold tracking-tight">Create Custom IAM Role</h3>
              <p className="text-xs text-indigo-200 mt-0.5">Configure fine-grained module access level rules.</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-xs font-extrabold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-3">
              Module Access Matrix
            </label>
            <div className="border border-slate-200 dark:border-slate-800 rounded-2xl divide-y divide-slate-100 dark:divide-slate-800 bg-slate-50/50 dark:bg-slate-950/40 max-h-72 overflow-y-auto">
              {MODULES.map((mod) => {
                ...
                return (
                  <div key={mod.key} className="p-3.5 flex items-center justify-between hover:bg-slate-100/50 dark:hover:bg-slate-800/30 transition-colors">
                    <div className="flex items-center gap-3">
                      {mod.icon}
                      <span className="text-sm font-bold text-slate-900 dark:text-slate-200">{mod.label}</span>
                    </div>
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition-all ${
                          currentLevel === 'none'
                            ...
                            : 'bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:text-slate-900 dark:hover:text-slate-200'
                        }`}
                      >
                        None
                      </button>
